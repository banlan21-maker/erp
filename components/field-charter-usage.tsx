...
export default function FieldCharterUsage() {
  ...
  // 운전자 이름 변경 — 마스터 일치 시 그 사람 차량/전화로 덮어씀.
  // MobileAutocomplete 의 목록 클릭 / 직접 타이핑 둘 다 onChange 로 들어옴.
  ...
  // 옵션: 차량번호 · 전화번호를 sub 에 표시 → 한눈에 식별
  const driverOptions = drivers.map(d => {
    const meta = [d.vehicleNo, d.phoneNo].filter(Boolean).join(" · ");
    return { label: d.name, sub: meta || undefined };
  });

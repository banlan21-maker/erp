      ...
      {!hideTabs && (
        <div className="flex border-b border-gray-200 gap-0 overflow-x-auto">
          {[
            ...
            { key: "remnants",  icon: <Trash2 size={14} />,       label: "등록잔재" },
          ].map(({ key, icon, label }) => (
            <button
              key={key}
              onClick={() => goTab(key)}
...
function RemnantListTab({
  projectOptions, activeProject, projectId, router, baseUrl = "/cutpart/drawings",
}: {
  projectOptions: ProjectOption[];
  ...
  type RemnantRow = {
    ...
    sourceBlock: string | null; createdAt: string;
  };
  ...
  if (!projectId || !activeProject) {
    return (
      <div className="space-y-2">
        <p className="text-sm text-gray-500">프로젝트(블록)를 선택하세요.</p>
        <div className="grid gap-2">
          {projectOptions.map(p => (
            <button key={p.id} onClick={() => router.push(`${baseUrl}?tab=remnants&projectId=${p.id}`)}
              className="text-left px-4 py-3 bg-white border rounded-xl hover:border-blue-400 hover:bg-blue-50 transition-colors text-sm">
              <span className="font-semibold text-gray-800">[{p.projectCode}]</span>
              <span className="ml-2 text-gray-600">{p.projectName}</span>
            </button>
          ))}
        </div>
      </div>
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-3">
        <button onClick={() => router.push(`${baseUrl}?tab=remnants`)}
          className="flex items-center gap-1.5 text-sm text-gray-500 hover:text-gray-700">
          <ArrowLeft size={15} /> 목록으로
        </button>
        <h3 className="text-base font-semibold text-gray-800">
          [{activeProject.projectCode}] {activeProject.projectName} — 등록잔재
        </h3>

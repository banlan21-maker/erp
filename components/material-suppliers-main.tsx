...
import {
  ...
  FileText, Search, Plus, List, Building, Trash2, Edit, Factory,
...
const FACTORY_BADGE: Record<FactoryKey, string> = {
...
// 리스트 컬럼 정의 — Project.MD § 13.X cascading filter 표준 패턴
const COLS = [
  { key: "name",      label: "거래처명",   align: "left"   as const, filterable: true },
  { key: "factory",   label: "담당공장",   align: "center" as const, filterable: true },
  { key: "manager",   label: "담당자",     align: "left"   as const, filterable: true },
  { key: "contact1",  label: "연락처1",    align: "left"   as const, filterable: true },
  { key: "email",     label: "이메일",     align: "left"   as const, filterable: true },
  { key: "bizNumber", label: "사업자번호", align: "left"   as const, filterable: true },
] as const;
...
export default function MaterialSuppliersMain() {
  ...
  // accessor map — Project.MD 표준 cascading filter 패턴
  const accessors = useMemo<ColumnAccessorMap<Supplier>>(() => ({
    name:      s => s.name,
    factory:   s => s.factory,
    manager:   s => s.manager,
    contact1:  s => s.contact1,
    email:     s => s.email,
    bizNumber: s => s.bizNumber,
  }), []);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData.name.trim()) { alert("거래처 이름은 필수입니다."); return; }
  ...
  // 검색어 (이름/담당자) → cascading 필터 → 정렬
  const searchFiltered = useMemo(() =>
    !searchTerm
      ? suppliers
      : suppliers.filter(s => s.name.includes(searchTerm) || s.manager.includes(searchTerm)),
              ...
              <div className="flex items-center gap-3 flex-wrap">
                ...
                {filterCount > 0 && (
                  ...
                  >
                    ...
                    필터 {filterCount}개 · 초기화
                  </button>
                )}
              </div>
              <span className="text-sm text-gray-500">
                총 <strong className="text-gray-900">{sortedRows.length}</strong>개의 거래처
              </span>
            </div>
                ...
                <thead className="bg-gray-50 text-gray-500 border-b border-gray-200">
                  <tr>
                    {COLS.map(col => {
                      ...
                      return (
                        <th key={col.key} className={`px-4 py-2 text-[11px] font-semibold text-${col.align} whitespace-nowrap`}>
                    ...
                    <th className="px-4 py-2 text-[11px] font-semibold text-center whitespace-nowrap">관리</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-100">
                  {sortedRows.length === 0 ? (
                    <tr>
                      <td colSpan={COLS.length + 1} className="px-6 py-12 text-center text-gray-500">
                    ...
                    sortedRows.map(supplier => (
                      ...
                      >
                        <td className="px-4 py-2 font-bold text-gray-900">{supplier.name}</td>
                        <td className="px-4 py-2 text-center">
                          <span className={`text-[11px] px-2 py-0.5 rounded-full border font-semibold ${FACTORY_BADGE[supplier.factory]}`}>
                            {supplier.factory}
                          </span>
                        </td>
                        <td className="px-4 py-2 text-gray-700">{supplier.manager || "-"}</td>
                        <td className="px-4 py-2 text-gray-700">{supplier.contact1 || "-"}</td>
                        <td className="px-4 py-2 text-gray-500">{supplier.email || "-"}</td>
                        <td className="px-4 py-2 text-gray-500">{supplier.bizNumber || "-"}</td>
          ...
          <div>
            ...
            <form onSubmit={handleSubmit} className="p-6 sm:p-8">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                ...
                {/* 메모 */}
                <div className="md:col-span-2">
                  <label className="block text-sm font-medium text-gray-700 mb-1.5">메모 (특이사항)</label>
                  <textarea
                    name="memo"
                    value={formData.memo}
                    onChange={handleInputChange}
              ...
              <div className="mt-8 pt-6 border-t border-gray-100 flex justify-end gap-3">
                <button
                  type="button"
                  onClick={() => { setFormData(initialFormState); setActiveTab("list"); }}
          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh] animate-in fade-in zoom-in-95 duration-200">
            <div className="p-5 border-b border-gray-100 flex justify-between items-center bg-gray-50/80">
              <h3 className="font-bold text-lg text-gray-900 flex items-center gap-2">
                ...
                {viewSupplier.name}
                <span className={`text-[11px] px-2 py-0.5 rounded-full border font-semibold ${FACTORY_BADGE[viewSupplier.factory]}`}>
                  {viewSupplier.factory}
                </span>
              </h3>
            ...
            <div className="p-6 overflow-y-auto w-full">
              <div className="grid grid-cols-2 gap-y-6 gap-x-8">
                <div>
                  <p className="text-xs text-gray-500 mb-1">담당공장</p>
                  <p className="text-sm font-medium text-gray-900">{viewSupplier.factory}</p>
                </div>
                <div>

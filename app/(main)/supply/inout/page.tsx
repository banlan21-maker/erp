...
import { PackageCheck, PackageMinus, AlertCircle, RefreshCw, History, Search,
  ChevronDown, ArrowLeftRight, Star, ShoppingCart, Trash2, CheckCircle2, X } from "lucide-react";
                ...
                >
                  {onToggleFavorite && (
                    <button type="button" onMouseDown={(e) => { e.stopPropagation(); onToggleFavorite(item, e); }}
                      className="shrink-0 p-0.5 rounded hover:bg-yellow-50" title={item.isFavorite ? "즐겨찾기 해제" : "즐겨찾기 등록"}>
...
export default function InOutPage() {
    ...
    async function loadConfig() {
      try {
        const [itemsRes, vendorsRes] = await Promise.all([
          fetch("/api/supply/items"), fetch("/api/supply/vendors"),
        ]);
  ...
  const addToCart = () => {
    ...
    const newItem: CartItem = {
      ...
      itemName: selItem ? selItem.name : formData.itemId,
      itemUnit: selItem?.unit ?? "",
      vendorId: formData.vendorId,
      vendorName: selVendor ? selVendor.name : "",
      date: formData.date,
      qty: formData.qty,
    ...
    // 리셋 항목 초기화, 고정 항목 유지
    if (topMode === "in") {
      // 리셋: itemId, qty / 유지: vendorId, date, person, memo
      setFormData(prev => ({ ...prev, itemId: "", qty: "" }));
    } else {
      // 리셋: itemId, qty, memo / 유지: date, person
  ...
  const submitCart = async () => {
    ...
    try {
      ...
      for (const c of cart) {
        ...
        if (topMode === "out" && data.data?.isWarning) hasWarning = true;
      }
      if (hasWarning) window.alert("⚠️ [경보] 일부 품목 재고가 발주 기준점 이하로 떨어졌습니다!");
      else alert(`${topMode === "in" ? "입고" : "출고"} ${cart.length}건 처리가 완료되었습니다.`);
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex items-center gap-1 bg-gray-100 p-1 rounded-lg border border-gray-200">
          {deptTabs.map(tab => (
            <button key={tab.value} type="button" onClick={() => setDeptFilter(tab.value)}
              className={`px-5 py-1.5 rounded-md text-sm font-semibold transition-all ${
                deptFilter === tab.value ? "bg-white shadow-sm text-gray-900 border border-gray-200" : "text-gray-500 hover:text-gray-700"
              }`}
            >
              {tab.label}
            </button>
      ...
      <div className="flex justify-center">
        <div className="bg-gray-100 p-1.5 rounded-full inline-flex border border-gray-200 shadow-inner">
          <button type="button" onClick={() => setTopMode("in")}
            className={`flex items-center gap-2 px-8 py-2.5 rounded-full font-bold text-sm transition-all ${isIn ? "bg-white text-emerald-700 shadow-sm border border-emerald-100" : "text-gray-500 hover:text-gray-700"}`}>
            <PackageCheck size={18} /> 품목 입고 (등록)
          </button>
          <button type="button" onClick={() => setTopMode("out")}
            className={`flex items-center gap-2 px-8 py-2.5 rounded-full font-bold text-sm transition-all ${!isIn ? "bg-white text-orange-700 shadow-sm border border-orange-100" : "text-gray-500 hover:text-gray-700"}`}>
      ...
      {/* 등록 폼 */}
      <div className={`rounded-xl shadow-sm border overflow-hidden ${isIn ? "border-emerald-200 bg-white" : "border-orange-200 bg-white"}`}>
        <div className={`p-4 border-b flex items-center justify-between ${isIn ? "bg-emerald-50/80 border-emerald-100 text-emerald-900" : "bg-orange-50/80 border-orange-100 text-orange-900"}`}>
          <h3 className="font-bold flex items-center gap-2">
            {isIn ? <PackageCheck size={20} /> : <PackageMinus size={20} />}
            {isIn ? "입고 매입 전표 작성" : "현장 출고 수불부 작성"}
          </h3>
        ...
        <div className="p-6 md:p-8">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-6">
            ...
            {isIn && (
              <div className="sm:col-span-2">
                <label className="block text-sm font-semibold text-gray-800 mb-1.5">
                  ...
                  <span className="ml-2 text-xs text-emerald-600 font-normal">담기 후 고정 유지</span>
                </label>
                <SearchableSelect
            ...
            <div>
              <label className="block text-sm font-semibold text-gray-800 mb-1.5">
                ...
                <span className={`ml-2 text-xs font-normal ${isIn ? "text-emerald-600" : "text-orange-500"}`}>담기 후 고정 유지</span>
              </label>
            ...
            <div>
              <label className="block text-sm font-semibold text-gray-800 mb-1.5">
                {isIn ? "입고(매입) 수량" : "지급(출고) 수량"} <span className="text-red-500">*</span>
                <span className="ml-2 text-xs text-gray-400 font-normal">담기 후 초기화</span>
              </label>
            ...
            <div>
              ...
              <Input required name="person" value={formData.person} onChange={handleChange}
            ...
            <div className="sm:col-span-2">
              <label className="block text-sm font-medium text-gray-700 mb-1.5">
                비고 / 메모 (옵션)
                <span className={`ml-2 text-xs font-normal ${isIn ? "text-emerald-600" : "text-gray-400"}`}>
                  {isIn ? "담기 후 고정 유지" : "담기 후 초기화"}
                </span>
              </label>
          ...
          <div className="mt-6 pt-5 border-t border-gray-100 flex justify-end">
            <Button type="button" onClick={addToCart}
              className={`px-10 text-sm font-bold shadow-sm ${isIn ? "bg-emerald-600 hover:bg-emerald-700" : "bg-orange-600 hover:bg-orange-700"}`}>
      ...
      {/* 장바구니 */}
      {cart.length > 0 && (
        <div className={`rounded-xl border shadow-sm overflow-hidden ${isIn ? "border-emerald-300" : "border-orange-300"}`}>
          <div className={`px-4 py-3 border-b flex items-center justify-between ${isIn ? "bg-emerald-50 border-emerald-200" : "bg-orange-50 border-orange-200"}`}>
            <h3 className={`font-bold flex items-center gap-2 text-sm ${isIn ? "text-emerald-800" : "text-orange-800"}`}>
              <ShoppingCart size={16} />
              장바구니 — {cart.length}건
              <span className="font-normal text-gray-500">반영 전 확인 후 아래 버튼을 눌러주세요.</span>
            </h3>
            ...
            <table className="w-full text-xs whitespace-nowrap">
              <thead className="bg-gray-50 border-b text-gray-500 uppercase">
                <tr>
                  ...
                  {isIn && <th className="px-4 py-2 text-left">매입거래처</th>}
                  <th className="px-4 py-2 text-left">{isIn ? "입고일" : "출고일"}</th>
                  <th className="px-4 py-2 text-right">{isIn ? "입고수량" : "출고수량"}</th>
                  <th className="px-4 py-2 text-left">{isIn ? "확인자" : "사용자"}</th>
                ...
                {cart.map((c, idx) => (
                  <tr key={c._key} className="hover:bg-gray-50">
                    <td className="px-4 py-2.5 font-semibold text-gray-800">
                      ...
                      {c.itemName}
                      <span className="ml-1 text-gray-400 font-normal">({c.itemUnit})</span>
                    </td>
                    {isIn && <td className="px-4 py-2.5 text-gray-600">{c.vendorName || "-"}</td>}
                    <td className="px-4 py-2.5 text-gray-600 font-mono">{c.date}</td>
                    <td className={`px-4 py-2.5 text-right font-bold ${isIn ? "text-emerald-700" : "text-orange-700"}`}>
                      {isIn ? "+" : "-"}{c.qty}
                    </td>
                    <td className="px-4 py-2.5 text-gray-600">{c.person}</td>
                    <td className="px-4 py-2.5 text-gray-400 max-w-[160px] truncate">{c.memo || "-"}</td>
                    <td className="px-4 py-2.5">
                      <button onClick={() => removeFromCart(c._key)}
                        className="text-gray-300 hover:text-red-500 transition-colors" title="제거">
                        <Trash2 size={13} />
                      </button>
                    </td>
                  </tr>
          ...
          <div className={`px-4 py-3 border-t flex justify-end ${isIn ? "bg-emerald-50/50 border-emerald-200" : "bg-orange-50/50 border-orange-200"}`}>
            <Button onClick={submitCart} disabled={submitting}
              className={`px-10 text-sm font-bold shadow-sm ${isIn ? "bg-emerald-600 hover:bg-emerald-700" : "bg-orange-600 hover:bg-orange-700"}`}>
              <CheckCircle2 size={16} className="mr-2" />
              {submitting ? "처리 중..." : isIn ? `입고 및 재고반영 (${cart.length}건)` : `출고 및 재고차감 (${cart.length}건)`}
            </Button>
          </div>
        </div>
      )}

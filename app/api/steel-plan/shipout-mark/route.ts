 ...
 *   { action: "mark",   items: [{ id, heatNo?, label? }] }  → shipoutMarkedAt=now, shipoutHeatNo, shipoutLabel
 *   { action: "unmark", ids: [id, ...] }                    → shipoutMarkedAt/shipoutHeatNo/shipoutLabel=null
 *
 *   label = 확정정보 표시 라벨(예: 매칭이름). 출고등록은 생략(호선코드로 대체).
  ...
  try {
    ...
    if (action === "unmark") {
      ...
      const { count } = await prisma.steelPlan.updateMany({
        where: { id: { in: ids } },
        data:  { shipoutMarkedAt: null, shipoutHeatNo: null, shipoutLabel: null },
      });
    ...
    const items = rawItems
      .map((it) => it as { id?: unknown; heatNo?: unknown; label?: unknown })
      .filter((it) => typeof it.id === "string")
      .map((it) => ({
        id: it.id as string,
        heatNo: it.heatNo ? String(it.heatNo).trim() : null,

      ...
      for (const v of vehicles) {
        ...
        for (const item of v.items) {
          ...
          if (isRemnantItem(item)) {
            ...
            // 여유원재는 프로젝트 강재 판번호 목록(SteelPlanHeat)을 건드리지 않는다(2026-09-22 정책).
            //   여유원재는 따로 관리하는 자재라 그 판번호가 프로젝트 목록에 없다(재고 78장 전부 미등록 실측).
            //   예전엔 여기서 같은 사양 WAITING 판을 FIFO 로 찍어 SHIPPED 로 넘겼는데, 정확 매칭이
            //   실패하면 남의 판번호를 대신 소진시키는 구조였다. 잔재 EXHAUSTED + 명세 heatNo 스냅샷으로 끝낸다.
            continue;
          }

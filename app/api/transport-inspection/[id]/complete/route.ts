...
// POST /api/transport-inspection/[id]/complete — 검사 완료 처리
export async function POST(
  ...
  try {
    ...
    const item = await prisma.transportInspectionItem.findUnique({ where: { id } });
    if (!item) {
    ...
    const [updatedItem, log] = await prisma.$transaction([
      prisma.transportInspectionItem.update({
        where: { id },
        data: {
      ...
      prisma.transportInspectionLog.create({
        data: {
          itemId: id,
    ...
    console.error("[POST /api/transport-inspection/[id]/complete]", error);

...
| **R12** | **판번호 매칭은 호선 무관 — 시리즈호선 유연성** *(2026-07-15 정책 확정, 2026-07-22 전 경로 확대)*. 판번호는 **철판 한 장의 고유번호**이고 호선은 "어느 호선 예산으로 입고됐나" 라는 꼬리표일 뿐 실물을 구분하지 않는다. 야드에는 같은 규격의 자매호선 철판이 섞여 쌓이므로, 판번호를 호선 안에 가둬 찾으면 실물을 집어 찍었을 때 "없다" 가 뜬다. 따라서 **판번호 → 후보 조회는 재질·두께·폭·길이만 매칭**하고 호선은 표시·정렬에만 쓴다. **적용 범위**(2026-07-22 기준 전부): 현장 작업일보 picker(`heat-options`) · 현장 출고담기(`shipout-field`) · 현장직접출고(`shipout-field-adhoc`) · PC 출고등록 후보(`heat-match`) · 출고 확정 판번호 소진(`shipments` POST) · 엑셀 업로드 매칭(`excel-upload`) · 절단완료 판번호 소진 폴백(`cutting-complete`). **화면 표시**: 입력 판번호와 다른 호선의 항목은 배지로 경고(작업일보 amber `타호선`, 출고 cyan `다른 호선`). 선택은 자유. | heat-options / shipout-field / shipout-field-adhoc / heat-match / shipments / excel-upload / cutting-complete |
| **R13** | **판번호 소진 = 선택한 재고 id 정확일치** *(P1, 2026-07-21)*. 작업일보에서 판번호를 **목록에서 고르면 그 재고(SteelPlanHeat)의 id 를 `CuttingLog.selectedHeatId` 로 보존**하고, `applyCuttingComplete` 는 이 id 로 **그 판을 정확히 CUT 소진**(글자대조 없음 → 타호선·오타·중복 heatNo desync 원천 차단). **복원 키는 `consumedHeatId`** — `selectedHeatId` 는 '의도', `consumedHeatId` 는 '실제로 소진한 판' 이며 `applyCuttingRestore` 는 후자로 되돌린다(2026-07-21 `b02e11a5`, 형제 판 과다복원 방지). ⚠ **현실**: 절단완료 2159건 중 목록 선택은 26건(1.2%)뿐이고 나머지는 직접 입력이다. 따라서 **실무를 지탱하는 것은 아래 R14 폴백**이지 R13 이 아니다. | cutting-complete.ts / cutting-logs POST / field-worklog.tsx / worklog-admin.tsx |
| **R14** | **판번호 소진 폴백 = 호선 우선, 없으면 호선 무관** *(2026-07-22, `89237129`)*. `selectedHeatId` 가 없을 때(직접 입력·레거시) `applyCuttingComplete` 는 ① 작업호선(또는 대체호선)에서 `heatNo`+사양으로 찾고 ② 못 찾으면 **호선 조건 없이** 다시 찾는다. ②가 없으면 대체호선을 지정하지 않고 옆 호선 철판을 쓴 경우 **아무 판번호도 소진되지 않아** 유령 WAITING 이 영구히 남고, 그 사양 재고가 어긋나 현장 출고가 "재고 없음" 으로 막힌다(실제 31건 누적, 잔존 7건). ⚠ **강재(SteelPlan) 차감은 여전히 작업호선에서 일어난다** — 대체호선 미지정 시 판번호는 맞고 강재 호선은 어긋나는 비대칭이 남는다. 근본 해소는 **도면에 대체호선을 지정**하는 것(§9 C). | cutting-complete.ts:123-156 |
| **R15** | **판번호 조회는 표기 정규화 폴백** *(2026-07-22, `dd3394c9`)*. 현장 입력창은 바코드 잡문자를 걸러내되 **하이픈은 통과**시킨다(`cleanHeatNo`). 서버는 정확 일치가 실패하면 **영문·숫자만 남긴 형태**로 한 번 더 찾는다(`lib/heat-lookup.ts`, `regexp_replace`). `SUS-4` / `SUS4` / `sus-4` / `SUS 4` 가 모두 같은 판을 가리킨다. 매칭되면 **DB 등록 표기를 응답에 실어** 출고장·거래명세표에 실물 라벨과 같은 표기가 찍히게 한다. 정규화 비교는 인덱스를 타지 않으므로 **정확 일치 실패 시에만** 호출한다. 현재 하이픈 판번호 72개(SUS-*·J-*·C25-*), 정규화 후 충돌 0건. | lib/heat-lookup.ts / field-shipout.tsx |
...
| API | `steel-plan/shipout-field` | `steel-plan/shipout-field-adhoc` | `steel-plan/shipout-match` · `heat-match` |
...
| 호선 조건 | **없음** (R12) | **없음** (R12) | **없음** (R12) |
| 감사 태그 | — | `ShipmentItem.adHocFromField = true` | — |

**후보 조회 공통 규칙**
1. 입력 판번호 → `SteelPlanHeat(status=WAITING)` 조회. 실패하면 **표기 정규화 폴백**(R15).
2. 못 찾으면 상태 불문 존재 여부로 `NOT_FOUND` / `ALREADY_USED` 구분.
3. 찾은 heat 의 **규격(재질·두께·폭·길이)** 만 취한다. **호선은 버린다**(R12).
4. 그 규격의 강재를 후보로 반환. **입력 판번호와 같은 호선을 목록 앞에** 두고, 다른 호선에는 `otherVessel = true` 를 실어 UI 가 배지를 단다.
5. 같은 판번호가 여러 규격에 등록된 경우(수입재) 그 규격들을 **전부** 후보 대상으로 본다(①). ②는 대표 규격 + `otherSpecs` 안내.

**후보 0건일 때 반드시 다음 행동을 제시할 것** — 막다른 길을 만들지 않는다.
- `reservedCount > 0` → "블록확정(절단용)이라 출고 불가. 프로젝트에서 확정취소 후 재시도"
- `notReceivedCount > 0` → "이 규격 N장은 이미 절단/출고 처리됨"
- 그 외 → 사양 폼을 열어 호선·규격을 직접 고쳐 재검색 (②는 `NOT_FOUND` 뿐 아니라 **후보 0건일 때도** 폼을 연다)
...
| 경로 | 조건 | 동작 |
|---|---|---|
| **매칭됨** | `steelPlanHeatId` 있음 | 그 heat 을 `WAITING → SHIPPED` 원자적 전환. 이미 바뀌었으면 롤백 |
| **직접입력** | `manualHeatNo = true` | ① 같은 호선에서 `heatNo`+규격 WAITING 조회 → ② 없으면 **호선 무관**으로 재조회 → ③ 그래도 없으면 **호선 무관 중복검사**(이미 CUT/SHIPPED 면 409 차단) → ④ 통과 시에만 신규 생성(`autoCreatedFromShipment=true`) |
| **미입력** | 판번호 없음 | 같은 규격 WAITING 1장을 FIFO 소진(강재↔판번호 개수 정합 유지) |

> **왜 ②③이 필수인가** — 이게 없으면 옆 호선 철판을 출고할 때 재고 판번호를 못 찾고, 중복검사도 통과해서, **잘못된 호선으로 같은 판번호를 새로 만든다.** 진짜 판번호는 WAITING 으로 남아 나중에 절단이나 다른 출고에 다시 소진된다.

**입구 방어도 같이 있어야 한다** (하나라도 빠지면 위 경로로 떨어진다):
- 후보 목록(`heat-match`)이 호선으로 잠겨 있으면 유용 강재 판번호가 **절대 안 뜨고** 반드시 손입력이 된다 → 호선 잠금 금지
- 후보 조회 실패를 `catch {}` 로 삼키면 **멀쩡한 상황에서도 목록이 비어** 손입력이 된다 → 반드시 화면에 노출
...
> 1. **호선은 실물을 구분하지 않는다.** 강재 행(SteelPlan)에는 호선이 붙지만 판번호는 규격 풀에 속한다. 그래서 판번호 조회에 호선을 걸면 안 된다(R12). 실측으로 출고담기 32.3%·현장직접출고 7.5%가 이 잠금 때문에 막혀 있었다.
...
야드에는 자매호선(KYTS-1022/1023) 같은 규격 철판이 **섞여 쌓인다**. 실측: 1022 대기 사양 814종 중
**773종이 1023과 규격 동일** — 실물로 구분 불가. 그런데 조회는 호선으로 잠겨 있었다.
...
| 증상 | 경로 | 실측 실패율 |
|---|---|---|
| 출고담기 "선별 강재 없음" | `shipout-field` | 666개 중 215개 (**32.3%**) |
| 현장직접출고 "입고 자재 없음" | `shipout-field-adhoc` | 3969개 중 288개 (7.5%) |
| PC 출고등록 후보 미표시 → 손입력 | `heat-match` | 968 규격 중 844종(87.2%)에서 후보 누락 |
...
(차단 215장의 마킹일: 07-16 84장 / 07-17 39장 / **07-21 92장**).
...
| 5 | PC 출고등록에서 **유령 판번호 생성 경로** (후보 잠금 → 손입력 → 잘못된 호선으로 신규 생성) | 후보·중복검사 호선 무관화 + `catch {}` 제거 + 엑셀 `HEAT_NOT_FOUND` | `ec7ba70e` |
| 6 | 하이픈 판번호 15장 **영구 조회 불가** (`SUS-4` → 입력창이 `SUS4` 로) | 입력창 하이픈 허용 + 서버 정규화 폴백 (R15) | `dd3394c9` |
...
- **"출고취소가 선별 마킹과 판번호 연결을 파괴한다"** → 기각. 정합성 검사 전 항목 0건:
  유령 출고 0/1568 · 유령 소진 0/1568 · 이중 출고 0 · 활성-원판 상태 불일치 0 · 보관위치 복원 10/10.
...
| LB4506 블록확정 | 입고 156장이 06-19~26 에 `LB4506/후행` 으로 전량 확정 → 현장직접출고 91% 차단. 실제 절단 물량인지 사무실 확인 필요(2026-07-22 현재 그대로 둠) |
| `heat-options` dead code | 제외 기준 `shipoutHeatNo ≠ null` 이 항상 비어 실질 무동작. 제거할지 PC 출고등록 흐름용으로 남길지 미정 |
| `ShipmentItem` XOR 미보장 | 스키마에 CHECK 제약 없음(둘 다 nullable). 테스트 데이터 2건 위반 존재(CANCELLED) |

### 15-5. 진단 도구 (`scripts/`, 로컬에서 DB 직접 실행)

**전부 읽기 전용**이며 `--apply` 가 붙은 것만 쓴다. 쓰기 스크립트는 실행 시 되돌리기 JSON 을 남긴다.
...
| `fix-crossvessel-plan.mjs [--apply]` | 호선 유용으로 뒤바뀐 재고 실물기준 교정 |
| `fix-selection-transfer.mjs [--apply]` | 위 교정의 잔여 선별 마킹 정리 |
...
## 변경 이력
- 2026-07-22: **§15 외부출고 전체 검증 신설** — 현장 신고 2건에서 출발해 6관점 병렬조사+적대적 반증으로 결함 7건 확정·조치, 오탐 2건 기각. **R12 전 경로 확대**(판번호 조회 호선 무관), **R14 신설**(소진 폴백 호선 우선→무관), **R15 신설**(표기 정규화 폴백). R13 복원 키를 `selectedHeatId` → `consumedHeatId` 로 정정. §12 에 출고 3경로 대조표·유령 판번호 방지 규칙 추가.

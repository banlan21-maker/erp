...
| **N3** | high | 동일 heatNo 다판(수입재) 절단 시 SteelPlanHeat.updateMany 대량 CUT → 원판 1:heat N desync | ⏳ 대기 |
| **N4** | high | 강재매칭이 reservedFor 제외 자재를 'NOT_RECEIVED'(미입고) 로 오안내 | ⏳ 대기 |
| **N5** | medium | reserve-bulk 프로젝트별 호출 강제 → A·B·C 순서 의존 배분 | ⏳ 대기 |
| **N6** | medium | shipout-mark 후 syncDrawingListBySpec 미호출 → 도면 status 스테일 | ⏳ 대기 |
| **N7** | medium | 등록잔재 자동 생성 없음 (문서 B6: 수동 정책) — 시나리오 기대와 갭 | ⏳ 대기 |
| **N8** | medium | 등록잔재 heatNo propagate 절단취소 시 청소 안 됨 → 스테일 데이터 | ⏳ 대기 |
| **N9** | medium | actualHeatNo FIFO 매핑 — 작업자 선택 heatNo 와 원판 소진 매핑 불일치 | ⏳ 대기 |
| **N10** | medium | 시나리오 8→9 논리 모순: 확정 유지 vs 외부출고 배타. UX 안내 부실 | ⏳ 대기 |
| **N11** | medium | 현장직접출고 후보 없음 안내 오진단 | ⏳ 대기 |
| **N12** | medium | 강재전체목록 [외부출고] alert 가 확정취소 경로 안내 없음 | ⏳ 대기 |
| **N13** | medium | 현장 [출고담기] 순환 안내 (사무실도 못하는 액션 요청) | ⏳ 대기 |
| **N14** | medium | 확정취소 시 특정 판재 지목 불가 (LIFO 자동) | ⏳ 대기 |

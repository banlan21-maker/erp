...
| R3 | **확정 매칭 우선순위**: 이 블록 예약(`호선/블록` 또는 `블록`) → 미예약(`null`) 폴백. 다른 블록 예약 강재는 절대 안 씀 | cutting-complete.ts |
...
### A. 소진·매칭 모델
- [ ] **A1. 원판 소진 = 도면1:원판1, FIFO 1장만 COMPLETED.** 같은 사양 여러 장 입고 시 절단 1건에 1장만 소진되고 나머지는 RECEIVED로 남음. → *맞나요?*
- [ ] **A2. 같은 판번호(heatNo)가 다른 호선·사양에 중복 존재 허용** (unique 제약 없음, 호선+사양으로 구분). → *맞나요?*
- [ ] **A3. 확정 매칭 우선순위 = 이 블록 예약 → 미예약 폴백.** 미예약 강재를 가져다 쓰는 게 맞는지, 아니면 항상 예약된 것만 써야 하는지?

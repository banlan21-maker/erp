...
- [ ] **B6. (신규) 정규(일반) 절단 시 '남은 잔재' 현장 등록 팝업 없음.** 코드 확인: 현장 정규 완료(`handleComplete`)는 돌발작업과 달리 "남은 잔재가 있나요?" 팝업이 없음. → **등록잔재**(강재리스트 업로드 시 사전선언분)만 절단완료 시 자동 IN_STOCK 전환되고, **현장잔재(즉석 발생 자투리)는 잔재관리에서 수동 등록**해야 함. 여유원재를 정규 절단할 때도 동일. **질문**: 여유원재/정규 절단 후 남는 잔재(특히 현장잔재)를 *현장에서 바로 등록*하는 팝업이 필요한가요? (현재는 돌발작업에만 있음)

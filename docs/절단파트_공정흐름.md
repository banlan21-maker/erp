...
| **R12** | **현장 판번호 매칭은 호선 무관 — 시리즈호선 유연성** *(2026-07-15 정책 확정)*. 현장 작업일보(`/field/worklog`) 의 판번호 picker (`/api/steel-plan/heat-options`) 는 `vesselCode` 조건을 걸지 않고 사양(재질·두께·폭·길이)만 매칭한다. 이유: 시리즈호선 특성상 도면 호선과 실물 판번호 호선이 다를 수 있음(예: A호선 도면에 B호선 판번호 재고 사용). **화면 표시**: 도면 호선과 다른 판번호는 amber(주황) 배지 `2222 · 타호선` 로 시각적 경고 (같으면 회색). 선택은 자유. | field-worklog.tsx / heat-options/route.ts |
| **R13** | **판번호 소진 = 선택한 재고 id 정확일치** *(P1, 2026-07-21)*. 현장/관리자 작업일보에서 판번호를 **목록에서 고르면 그 재고(SteelPlanHeat)의 id 를 `CuttingLog.selectedHeatId` 로 보존**하고, `applyCuttingComplete` 는 이 id 로 **그 판을 정확히 CUT 소진**(글자대조 없음 → 타호선·오타·중복 heatNo 로 인한 판번호 미소진 desync 원천 차단). `selectedHeatId` 가 없으면(레거시 로그·여유원재 자유입력) 기존 `heatNo`+effectiveVessel+사양 매칭으로 폴백. 복원(`applyCuttingRestore`)도 `selectedHeatId` 로 대칭 복원. **전제**: 정규작업 판번호는 목록 선택 전용(직접 타이핑 금지) — 그래야 선택=실제 재고가 보장됨. R12 의 "타호선 판번호 desync 이론적 가능성"은 R13 으로 해소됨. | cutting-complete.ts / cutting-logs POST / field-worklog.tsx / worklog-admin.tsx |
...
### 판번호 = 사양 매칭 추적용 (물리 지목 아님)  *(N21 재언급, 2026-07-15)*
§3 A2 정책 재요약 — 판번호(heatNo) 는 원판 1장의 고유 식별자 의미이지만, **시스템 상 강재(SteelPlan) 와 판번호(SteelPlanHeat) 는 느슨 연결**. 강재매칭이 heatNo → spec → SteelPlan(FIFO oldest) 로 임의 판 반환. 같은 사양이 여러 장이면 시스템이 고른 판재와 사용자 손의 물리 판재가 다를 수 있음. 재고 정합성은 사양 단위 카운트로 정확.

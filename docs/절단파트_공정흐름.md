...
> - 작성 기준: 2026-06-26 코드. **최근 갱신 2026-07-22** (변경 이력은 맨 아래)
> - **외부출고 현재 동작은 §6 불변식(R1·R12~R15) · §12 · §15 를 먼저 볼 것** — 2026-07-22 대폭 갱신·자체검증 완료.
...
| **R12** | **판번호 매칭은 호선 무관 — 시리즈호선 유연성** *(2026-07-15 정책 확정, 2026-07-22 대부분 경로 확대)*. 판번호는 **철판 한 장의 고유번호**이고 호선은 "어느 호선 예산으로 입고됐나" 라는 꼬리표일 뿐 실물을 구분하지 않는다. 야드에는 같은 규격의 자매호선 철판이 섞여 쌓이므로(실측 1022 대기사양의 ~95%가 1023과 규격 동일), 판번호를 호선 안에 가둬 찾으면 실물을 집어 찍었을 때 "없다" 가 뜬다(실측 출고담기 ~31% 차단). 따라서 **판번호 → 후보 조회는 재질·두께·폭·길이만 매칭**하고 호선은 표시·정렬에만 쓴다. **화면 표시**: 입력 판번호와 다른 호선의 항목은 배지로 경고(작업일보 amber `타호선`, 출고 cyan `다른 호선`). 선택은 자유. <br>**적용 범위 (2026-07-22 기준 — ⚠ 미완)**: ✅ 현장 출고담기(`shipout-field`) · 현장직접출고 판번호모드(`shipout-field-adhoc`) · PC 출고 판번호목록(`heat-match`) · 출고 확정 **직접입력** 갈래(`shipments` POST) · 엑셀 업로드(`excel-upload`) · 절단완료 소진 **정규원재** 폴백(`cutting-complete` R14) · 현장 작업일보(`field-worklog`→heat-options, 호출부 미전달). ❌ **아직 호선 잠금 잔존**: PC 출고등록 후보·중복검사(`shipout-match`, route.ts:44·58) · 출고 확정 **미입력 FIFO** 갈래(`shipments` route.ts:428) · 여유원재(SURPLUS) 절단 소진(`cutting-complete` :164-200) · 사무실 작업일보(`worklog-admin.tsx:588`, 의도적 호선격리). "전부"가 아니다 — §15-4 잔여과제. | heat-options / shipout-field / shipout-field-adhoc / heat-match / shipments(직접입력) / excel-upload / cutting-complete(R14) |
...
| 호선 조건 | **없음** (R12) | **없음** (R12) | ❌ **호선 잠금 잔존** `vesselCode: heat.vesselCode`(route.ts:58) — R12 미적용 |
| 정규화 폴백 R15 | ✅ | ✅ | ❌ 정확일치만 |
| 감사 태그 | — | `ShipmentItem.adHocFromField = true` | — |

> **③ `shipout-match` 는 아직 호선으로 잠겨 있다** (2026-07-22 미해결). 후보 조회(route.ts:57-73)·중복검사(:41-49)·RESERVED 판정이 모두 `vesselCode: heat.vesselCode` 를 건다. 그래서 PC 출고등록만은 유용 강재 판번호를 못 찾아 손입력으로 떨어진다. §15-4 잔여과제.
>
...
| **미입력** | 판번호 없음 | **같은 호선+규격** WAITING 1장 FIFO 소진(route.ts:428 — **호선 무관 재시도 없음**). ⚠ 이 갈래만 호선 조건이 남아 R12 예외. 타 호선에만 재고 판번호가 있으면 아무것도 소진되지 않고 통과 |
...
- 후보 목록(`heat-match`)이 호선으로 잠겨 있으면 유용 강재 판번호가 **절대 안 뜨고** 반드시 손입력이 된다 → 호선 잠금 금지 *(✅ heat-match 는 해제됨. ❌ shipout-match 는 아직 잠김)*
...
- ❌ **아직 뚫린 구멍**: 출고 확정(`POST /api/shipments`) 손입력 조회는 **정확일치만** — R15 정규화 폴백 미적용. 하이픈 표기차(`SUS-4`↔`SUS4`)면 ①②③ 빗나가 ④ 신규생성으로 떨어진다(WAITING 하이픈 15장). §15-4 잔여과제.
...
- **"출고취소가 선별 마킹과 판번호 연결을 파괴한다"** → 기각. 정합성 검사(2026-07-22) 전 항목 **0건**:
  ...
  취소 시 `shipoutMarkedAt` 을 지우므로 자동 재선별도 없다(`shipoutLabel` 만 추적용 유지).
...
| 항목 | 내용 |
|---|---|
| **`shipout-match` 호선 잠금 미해제** | PC 출고등록(강재전체목록·선별목록) 판번호 매칭·중복검사가 아직 `vesselCode: heat.vesselCode`(route.ts:44·58)로 잠겨 있어 유용 강재 후보를 못 찾아 손입력으로 떨어진다. R12 미적용. `heat-match`(출고장 만들기 모달)만 해제됨 |
| **출고 확정 손입력에 R15 미적용** | `POST /api/shipments` 직접입력 조회가 정확일치만 — 하이픈 표기차면 신규 판번호 생성으로 떨어진다(WAITING 하이픈 15장). `lib/heat-lookup.ts` 를 이 경로에도 적용 필요 |
| **여유원재(SURPLUS) 소진 호선 잠금** | `cutting-complete.ts:164-200` 은 `vesselCode: effectiveVessel` 로 잠기고 미발견 시 신규 heat 생성. R14 는 정규원재 경로만 |
| **미입력 FIFO 갈래 호선 고정** | `shipments` route.ts:428 미입력 판번호 FIFO 소진이 `vesselCode: item.vesselCode` 고정 — 타 호선에만 재고 판번호가 있으면 아무것도 소진 안 됨(R12 예외) |
| **`worklog-admin` 호선 잠금** | 사무실 작업일보(`worklog-admin.tsx:588`)는 `vesselCode` 를 전달해 호선 격리 상태(주석상 의도적). 현장 작업일보(field-worklog)는 미전달로 R12 적용 — 두 화면이 다르게 동작 |
| **R14 동명 판번호 오소진 위험** | R14 폴백이 heatNo+사양만 맞으면 호선 무관 소진 — 수입재 등 같은 판번호가 2장 이상이면 엉뚱한 판이 CUT 될 수 있음 |

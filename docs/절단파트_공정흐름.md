...
### B. 잔재
- [ ] **B1. (문서 정정됨) 여유원재(SURPLUS) 절단 = EXHAUSTED 소진.** *최초 문서의 "IN_STOCK 유지"는 코드 오독이었음(2026-06-26 정정).* 실제 코드: 여유원재를 도면에 지정해 절단 완료하면 **EXHAUSTED(소진)** + 실물 판번호가 판번호목록(SteelPlanHeat)에 CUT으로 기록(등록잔재와 동일 + 판번호 추적). → **질문**: 여유원재 절단 시 *전량 소진*이 맞나요, 아니면 큰 여유 원판이라 자르고 **남은 부분을 재고로 남겨야** 하나요(부분 사용)? 그리고 현장에서 '여유원재'는 정확히 어떤 재고를 뜻하나요?

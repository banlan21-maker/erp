...
if errorlevel 1 (
    ...
    echo.
    echo   2. During installation, check BOTH boxes:
    echo      [v] Add python.exe to PATH
    echo      [v] py launcher
    echo.
    echo   3. Close this window and re-run install.bat
    echo.
    echo Alternative without Python 3.11: use pytesseract.
...
if not exist "venv\Scripts\python.exe" (
    echo [STEP 1 of 3] Creating Python 3.11 virtual environment in .\venv ...
    py -3.11 -m venv venv
    if errorlevel 1 (
...
echo [STEP 2 of 3] Upgrading pip in venv...
"venv\Scripts\python.exe" -m pip install --upgrade pip
echo.

REM --- 4. Install packages in venv ---
echo [STEP 3 of 3] Installing packages from requirements.txt
...
if errorlevel 1 (
    ...
    echo.
    echo   2. Network or proxy issue - retry install.bat
    echo.
